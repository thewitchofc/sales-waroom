...
export function ProfilePageContent() {
  ...
  const nextTier =
    RANK_TIERS[RANK_TIERS.findIndex((t) => t.id === user.tier) + 1];
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-6">
          ...
          <section className="border border-white/5 bg-black/60 p-5 md:p-6">
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <StatBox
                label="אחוז ניצחון"
                value={`${Math.round((user.wins / (user.wins + user.losses)) * 100)}%`}
                accent
                large
              />
            </div>
        ...
        <div className="space-y-5">
          <section className="border border-white/5 bg-black/60 p-5">
            ...
            {nextTier && (
              <p className="mt-3 text-xs text-muted-foreground">
                {(nextTier.minXp - user.xp).toLocaleString()} XP עד{" "}
                {nextTier.labelHe}
              </p>
            )}
          </section>
          ...
          <section className="border border-accent/20 bg-accent/5 p-5">
            <div className="mb-3 font-brand text-[10px] text-accent">
              פריסה מהירה
            </div>
      ...
      <section className="border border-white/5 bg-black/40 p-5">
        <div className="mb-4 font-brand text-[10px] text-muted-foreground">
          סולם דרגות
        </div>
