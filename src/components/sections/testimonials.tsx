  ...
  return (
    ...
    >
      <div className="mb-4 flex flex-wrap items-start justify-between gap-2 border-b border-white/5 pb-4">
        <div>
          <div className="font-brand text-[9px] text-muted-foreground">דוח לוחם · {report.logId}</div>
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 text-[10px]">
        <div className="border border-white/5 bg-white/[0.02] px-3 py-2">
          ...
          <div className="mt-0.5 font-semibold text-white">{report.outcome}</div>
        </div>
        <div className="border border-white/5 bg-white/[0.02] px-3 py-2">
          <div className="text-muted-foreground">שינוי</div>
          <div className="mt-0.5 font-semibold text-green-400/90">{report.delta}</div>
        </div>
      </div>

      <p className="mb-6 flex-1 text-sm leading-[1.75] text-white/80 sm:text-base">{report.quote}</p>

      <div className="flex items-center justify-between border-t border-white/5 pt-4">
        <div>
          <div className="text-sm font-semibold text-white">{report.operator}</div>
          <div className="text-[10px] text-muted-foreground">{report.unit}</div>
        </div>
        <span className="font-brand text-[10px] text-accent">{report.metric}</span>
      </div>
    </GlassCard>
  );
}
...
export function Testimonials() {
  ...
  return (
    <Section id="testimonials" className="border-t border-white/5 py-16 sm:py-20">
      <div className="mb-10 grid gap-6 lg:grid-cols-12 lg:items-end">
        <div className="lg:col-span-7">
          <p className="mb-2 font-brand text-[10px] tracking-[0.2em] text-accent">דוחות לוחמים</p>
      ...
      <div className="grid gap-5 lg:grid-cols-12 lg:gap-6">
        <ReportCard report={featured} index={0} className="lg:col-span-7 lg:min-h-[320px]" />
